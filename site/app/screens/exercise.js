...
export async function renderExercise(ctx, sessionId, exerciseId) {
  ...
  const tabHistBtn = h('button', { class: 'ex-tab', onClick: () => switchTab('hist') }, 'History',
    histCount ? h('span', { class: 'count' }, String(histCount)) : null);
  const tabStatsBtn = h('button', { class: 'ex-tab', onClick: () => switchTab('stats') }, 'Stats');
  function switchTab(which) {
    tabLogBtn.classList.toggle('active', which === 'log');
    tabHistBtn.classList.toggle('active', which === 'hist');
    tabStatsBtn.classList.toggle('active', which === 'stats');
    logPane.hidden = which !== 'log';
    histPane.hidden = which !== 'hist';
    statsPane.hidden = which !== 'stats';
  }
  ...
  return h('div', { class: 'screen' }, scroll,
    h('div', { class: 'ex-tabs' }, tabLogBtn, tabHistBtn, tabStatsBtn),
    bottomNav('feed', ctx));
}
